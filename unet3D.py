...
def conv3d(net, D, H, W, output_dim, is_training, acitvation, layer_name):
...
def transpose3d(net, D, H, W, output_shape, is_training,acitvation, layer_name):
    with tf.variable_scope(layer_name):
        output_dim = output_shape[-1]
        w = tf.get_variable('w', [D, H, W, output_dim, net.get_shape()[-1]],
                            initializer=tf.contrib.layers.xavier_initializer())
        trans = tf.nn.conv3d_transpose(net, w, output_shape, strides=[1, 2, 2, 2, 1], padding='SAME')
...
def residualblock3D(net, output_dim, is_training, layer_name):
     with tf.variable_scope(layer_name):
        net = conv3d(net, 1, 3, 3, output_dim, is_training, 'elu', 'conv%d_1' % int(layer_name[-1]))
        shortcut = net
        net = conv3d(net, 3, 3, 3, output_dim, is_training, 'elu', 'conv%d_2' % int(layer_name[-1]))
        net = conv3d(net, 3, 3, 3, output_dim, is_training, 'None', 'conv%d_3' % int(layer_name[-1]))
...
def Unet3D(inputs,
    ...
    with tf.variable_scope(scope, 'unet3D', [inputs]) as sc:
        ...
        # downsampling  path #
        ######################
        conv1_1 = conv3d(inputs, 3, 3, 3, 32, is_training, 'relu', 'conv1/conv1_1')
        conv1_2 = conv3d(conv1_1,3, 3, 3, 64, is_training, 'relu', 'conv1/conv1_2')
        pool1 = tf.nn.max_pool3d(conv1_2,ksize=[1, 2, 2, 2, 1],
                                        strides=[1, 2, 2, 2, 1],
                                        padding='SAME')

        conv2_1 = conv3d(pool1,3, 3, 3, 64, is_training, 'relu', 'conv2/conv2_1')
        conv2_2 = conv3d(conv2_1,3, 3, 3, 128, is_training, 'relu', 'conv2/conv2_2')
        pool2 = tf.nn.max_pool3d(conv2_2,ksize=[1, 2, 2, 2, 1],
                                        strides=[1, 2, 2, 2, 1],
                                        padding='SAME')

        conv3_1 = conv3d(pool2,3, 3, 3, 128, is_training, 'relu', 'conv3/conv3_1')
        conv3_2 = conv3d(conv3_1,3, 3, 3, 256, is_training, 'relu', 'conv3/conv3_2')
        ...
        # bottleneck #
        ##############
        conv4_1 = conv3d(pool3,3, 3, 3, 256, is_training, 'relu', 'conv4/conv4_1')
        conv4_2 = conv3d(conv4_1,3, 3, 3, 512, is_training, 'relu', 'conv4/conv4_2')
        ...
        transpose_output_shape = [tf.shape(conv4_2)[0], shape[1] * 2, shape[2] * 2,
                                   shape[3] * 2, 512]
        conv5_1 = transpose3d(conv4_2, 2, 2, 2, transpose_output_shape, is_training, 'relu', 
                                                                    'conv5/transpose_conv5_1')
        merge_1 = tf.concat([conv5_1, conv3_2], axis=-1, name='merge1') 
        conv5_2 = conv3d(merge_1,3, 3, 3, 256, is_training, 'relu', 'conv5/conv5_2')
        conv5_3 = conv3d(conv5_2,3, 3, 3, 256, is_training, 'relu', 'conv5/conv5_3')

        shape = conv5_3.get_shape().as_list()
        transpose_output_shape = [tf.shape(conv5_3)[0], shape[1] * 2, shape[2] * 2,
                                   shape[3] * 2, 256]
        conv6_1 = transpose3d(conv5_3, 2, 2, 2, transpose_output_shape, is_training, 'relu', 
                                                                    'conv6/transpose_conv6_1')
        merge_2 = tf.concat([conv6_1, conv2_2], axis=-1, name='merge2') 
        conv6_2 = conv3d(merge_2,3, 3, 3, 128, is_training, 'relu', 'conv6/conv6_2')
        conv6_3 = conv3d(conv6_2,3, 3, 3, 128, is_training, 'relu', 'conv6/conv6_3')

        shape = conv6_3.get_shape().as_list()
        transpose_output_shape = [tf.shape(conv6_3)[0], shape[1] * 2, shape[2] * 2,
                                   shape[3] * 2, 128]
        conv7_1 = transpose3d(conv6_3, 2, 2, 2, transpose_output_shape, is_training, 'relu', 
                                                                    'conv7/transpose_conv7_1')
        merge_3 = tf.concat([conv7_1, conv1_2], axis=-1, name='merge3') 
        conv7_2 = conv3d(merge_3,3, 3, 3, 64, is_training, 'relu', 'conv7/conv7_2')
        conv7_3 = conv3d(conv7_2,3, 3, 3, 64, is_training, 'relu', 'conv7/conv7_3')
...
def ResidualUnet3D(inputs,
    ...
    with tf.variable_scope(scope, 'residualUnet3D', [inputs]) as sc:
        ...
        # downsampling  path #
        ######################
        input_conv = conv3d(inputs, 1, 5, 5, 28, is_training, 'elu', 'input_conv')
        ...
        transpose_output_shape = [tf.shape(conv6)[0], shape[1], shape[2] * 2,
                                   shape[3] * 2, 64]
        conv7 = transpose3d(conv6, 1, 2, 2, transpose_output_shape, is_training, 'None', 
                                                                    'conv7/transpose_conv')
        conv7 = tf.nn.elu(conv7 + conv3)
        ...
        transpose_output_shape = [tf.shape(conv7)[0], shape[1], shape[2] * 2,
                                   shape[3] * 2, 48]
        conv8 = transpose3d(conv7, 1, 2, 2, transpose_output_shape, is_training, 'None', 
                                                                    'conv8/transpose_conv')
        conv8 = tf.nn.elu(conv8 + conv2)
        ...
        transpose_output_shape = [tf.shape(conv8)[0], shape[1], shape[2] * 2,
                                   shape[3] * 2, 36]
        conv9 = transpose3d(conv9, 1, 2, 2, transpose_output_shape, is_training, 'None', 
                                                                    'conv9/transpose_conv')
        conv9 = tf.nn.elu(conv9 + conv1)
        conv9 = residualblock3D(conv9, 28, is_training, 'conv9')
        output_conv = conv3d(conv9, 1, 5, 5, 28, is_training, 'elu', 'output_conv')
