...
def train(device):
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope('unet2D') as unet2D_scope:
            with tf.name_scope('train') as train_scope:
                ...
                with slim.arg_scope(unet.unet_arg_scope()):
                    train_logits, end_points = unet.Unet(train_processed_images,
                                                is_training=True,
            ...
            unet2D_scope.reuse_variables() # training variables are reused in validation graph 
                ...
                while not coord.should_stop():
                    ...
                    if step_count % config.validate_every_num_steps == 0:
                        ...
                        if val_a_rand_total <= val_a_rand_min:
                            file_name = 'unet2D_{0}_{1}'.format(dt_stamp, step_count)
                            saver.save(
                                sess,
