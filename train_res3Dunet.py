...
def train(device):
    ...
    train_volume = volume[:80,:,:]
    train_affinities = affinity[:80,:,:]

    test_volume = volume[80:,:,:]
    test_affinities = affinity[80:,:,:]

    print np.shape(test_volume)
    print np.shape(test_affinities)

    with tf.Graph().as_default():

        # defining model names and setting output and summary directories
        model_train_name = 'resUnet3D'
        ...
        train_volume = tf.placeholder(tf.float32, shape=[None,18,160,160], name='train_volume')
        train_aff = tf.placeholder(tf.int32, shape=[None,18,160,160], name='train_aff')

        val_volume = tf.placeholder(tf.float32, shape=[None,18,160,160], name='val_volume')
        val_aff = tf.placeholder(tf.int32, shape=[None,18,160,160], name='val_aff')
        ...
        with tf.variable_scope('resUnet3D') as res_unet_scope:
            ...
            res_unet_scope.reuse_variables() # training variables are reused in validation graph 

            with tf.name_scope('val') as val_scope:

                val_logits = unet3D.Unet3D(val_images,
                                        is_training=False,
                                        num_classes = config.output_shape,
                                        scope=res_unet_scope)
                ...
                val_a_rand = config.a_rand(val_scores, val_masks)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for _ in range(config.iters):
                ...
                crop_train_volume, indiciies = random_crop_volume(train_volume,config.crop_size)
                crop_train_affinities = crop_volume(crop_train_affinities,config.crop_size, indiciies)
                ...
                if step_count % config.validate_every_num_steps == 0:
                    ...
                    for num_vals in range(config.num_batches_to_validate_over):
                        ...
                        crop_val_volume, indiciies = random_crop_volume(val_volume,config.crop_size)
                        crop_val_affinities = crop_volume(crop_val_affinities,config.crop_size, indiciies)
                    ...
                    if val_a_rand_total <= val_a_rand_min:
                        file_name = 'resUnet3D_{0}_{1}'.format(dt_stamp, step_count)
                        saver.save(
                            sess,
